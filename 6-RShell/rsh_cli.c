...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1)
    {
        ...
        {
            // Exit command, break the main loop to exit client
            break;
        }
        else if (strncmp(cmd_buff, "stop-server", strlen("stop-server")) == 0)
        {
            // Stop server command, break the main loop to exit client
            break;
        }
    }
